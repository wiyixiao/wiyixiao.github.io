...
GMenu::GMenu(GOled *oled)
{
	this->oled = oled;
	item_h = 12; //默认高度
	item_w = oled->width(); //默认宽度
	curItem = 0;
	itemSelect = 0;
}
...
void GMenu::drawMenu()
{
	...
	for (i = 0; i < itemNum; i++)
	{
		...
		if (item < itemNum)
		{
			...
			if (curItem == i)
			{
				oled->drawRect(0, max(0, i*(item_h + 1)), item_w, item_h, WHITE);
			}
...
void GMenu::itemChanged(byte dir)
{
	...
	oled->drawRect(0, max(0, curItem*(item_h + 1)), item_w, item_h, WHITE);
	oled->display();
